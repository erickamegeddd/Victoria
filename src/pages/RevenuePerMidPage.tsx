...
const fmt = (n) => n != null ? `$${Number(n).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}` : "ÃÂ¢ÃÂÃÂ";
...
const RevenuePerMidPage = () => {
  ...
  const fetchData = async () => {
    ...
    rows.forEach(r => {
      ...
      if (!map[key]) map[key] = {
        ...
        iso_name: r.isos?.name || "ÃÂ¢ÃÂÃÂ",
        iso_id: r.iso_id,
        total_net: 0,
        total_volume: 0,
        total_gross: 0,
  ...
  const columns = [
    ...
    { title: "MID", dataIndex: "mid", key: "mid", width: 160, filters: [...new Set(data.map(r=>r.mid).filter(Boolean))].sort().map(n=>({text:n,value:n})), onFilter: (v,r)=>r.mid===v },
    { title: "Business Name", dataIndex: "business_name", key: "dba", ellipsis: true, filters: [...new Set(data.map(r=>r.business_name).filter(Boolean))].sort().map(n=>({text:n,value:n})), onFilter: (v,r)=>r.business_name===v,
    ...
    { title: "Last Report", dataIndex: "last_month", key: "last", width: 110, align: "center", sorter: (a, b) => (a.last_month||"").localeCompare(b.last_month||""),
      render: v => v ? dayjs(v).format("MMM YYYY") : "ÃÂ¢ÃÂÃÂ" },
  ];
    ...
    <div>
      ...
      <Card style={{ marginBottom: 12 }}>
        <Space wrap>
          ...
          <Text style={{ color: "var(--muted-color)", fontSize: 12 }}>{processingMids.length} processing MIDs ÃÂ· {filteredData.length - processingMids.length} gateway MIDs</Text>
        </Space>
      </Card>
      <Card>
