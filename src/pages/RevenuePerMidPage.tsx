...
const RevenuePerMidPage = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    let base = supabase.from("residuals").select("*,isos(id,name)").eq("report_month", selectedMonth).order("report_month", { ascending: true });
  ...
  const columns = [
    ...
    { title: "MID", dataIndex: "mid", key: "mid", width: 160,
      ...
      filters: [...new Set(data.map(r=>r.mid).filter(Boolean))].sort().map(n=>({text:n,value:n})), onFilter: (v,r)=>r.mid===v },
    { title: "Business Name", dataIndex: "business_name", key: "dba", ellipsis: true,
      ...
      filters: [...new Set(data.map(r=>r.business_name).filter(Boolean))].sort().map(n=>({text:n,value:n})), onFilter: (v,r)=>r.business_name===v,
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        ...
        <Text style={{ color: "var(--muted-color)", fontSize: 12 }}>{processingMids.length} processing · {filteredData.length - processingMids.length} gateway</Text>
      </div>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={8}><Card><Statistic title="Total Net Residual Income" value={totalNet} prefix="$" precision={2} valueStyle={{ color: "var(--primary-color)", fontWeight: 700 }} formatter={v => Number(v).toLocaleString("en-US",{minimumFractionDigits:2})}/></Card></Col>
