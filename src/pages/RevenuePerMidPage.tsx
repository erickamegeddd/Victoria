...
const fmt = (n) => n != null ? `$${Number(n).toLocaleString("en-US",{minimumFractionDigits:2,maximumFractionDigits:2})}` : "Ã¢ÂÂ";
...
const RevenuePerMidPage = () => {
  ...
  const columns = [
    ...
    { title: "MID", dataIndex: "mid", key: "mid", width: 160, ...getSearchProps("mid", "MID"), filters: [...new Set(data.map(r=>r.mid).filter(Boolean))].sort().map(n=>({text:n,value:n})), onFilter: (v,r)=>r.mid===v },
    { title: "Business Name", dataIndex: "business_name", key: "dba", ellipsis: true, ...getSearchProps("business_name", "Business Name"), filters: [...new Set(data.map(r=>r.business_name).filter(Boolean))].sort().map(n=>({text:n,value:n})), onFilter: (v,r)=>r.business_name===v,
    ...
    { title: "Last Report", dataIndex: "last_month", key: "last", width: 110, align: "center", sorter: (a, b) => (a.last_month||"").localeCompare(b.last_month||""),
      render: v => v ? dayjs(v).format("MMM YYYY") : "Ã¢ÂÂ" },
  ];
    ...
    <div>
      ...
      <Card style={{ marginBottom: 12 }}>
        <Space wrap>
          ...
          <Text style={{ color: "var(--muted-color)", fontSize: 12 }}>{processingMids.length} processing MIDs Â· {filteredData.length - processingMids.length} gateway MIDs</Text>
        </Space>
      </Card>
      <Card>
