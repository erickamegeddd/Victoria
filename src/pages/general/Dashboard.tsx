...
const Dashboard = () => {
  ...
  const droppedFromPrevMonth = prevResiduals.filter(r=>!isGatewayRow(r)&&!isAggregateMid(r.mid)&&r.paydiversenet&&!currentMidSet.has(r.mid)).length;
  ...
  const monthlyView = (
    <>
      ...
      <Row gutter={16} style={{marginBottom:12}}>
        ...
        <Col span={8}><div style={{background:'#fef3c7',border:'1.5px solid #fbbf24',borderRadius:10,padding:'20px 24px',boxShadow:'0 1px 4px rgba(0,0,0,0.06)'}}><div style={{fontSize:14,fontWeight:800,textTransform:'uppercase',letterSpacing:'0.5px',color:'var(--black-color)',marginBottom:10}}>Active Merchants</div><div><span style={{fontSize:26,fontWeight:800,color:'var(--primary-color)'}}>{activeMids}</span><span style={{fontSize:13,fontWeight:600,color:'var(--muted-color)',marginLeft:6}}>active</span><span style={{fontSize:13,fontWeight:700,color:'#dc2626',marginLeft:10}}> | {droppedFromPrevMonth} dropped from last month</span></div></div></Col>
      </Row>
