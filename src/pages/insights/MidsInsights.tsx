...
const MidInsights = () => {
  ...
  const fetchUniqueMidsData = async () => {
    const { data } = await client.get<any[]>("/unique-mid-dba");
    return data;
  };

  const {
    data: uniqueMids,
    isLoading: isUniqueMidsLoading,
    error: uniqueMidsError,
  } = useQuery("fetchUniqueMidsData", fetchUniqueMidsData);

  const fetchRevenuePerMid = async () => {
    const { data } = await client.get<Columns[]>(`/mid-insights`, {
      params: { start_date: startDate, end_date: endDate, mid: selectedMid },
    });
  ...
  const {
    data: midData,
    isLoading: midDataLoading,
    error: midError,
  } = useQuery(
    ["fetchRevenuePerMid", startDate, endDate, selectedMid],
    fetchRevenuePerMid,
    {
      enabled: !!startDate && !!endDate && !!selectedMid,
    }
  );
  ...
  const handleDownload = () => {
    ...
    XLSX.writeFile(
      workbook,
      `MID-Insights-${dayjs(startDate).format("MMMM-YYYY")}-to-${dayjs(
        endDate
  ...
  if (uniqueMidsError || midError) message.error("Error fetching MID's");
  return (
    <>
      <Spinner isLoading={isUniqueMidsLoading || midDataLoading} />
      <Row gutter={[16, 16]}>
        <Col xxl={6} xl={6} lg={6} md={24} sm={24} xs={24}>
          <Select
            ...
            filterOption={
              (input: any, option: any) =>
                (option?.label ?? "")
                  .toLowerCase()
                  .includes(input.toLowerCase()) || // Search by DBA
                (option?.value ?? "")
                  .toLowerCase()
          ...
          >
            {uniqueMids?.map((opt: any, index: number) => (
              <Option key={index} value={opt.mid} label={opt.dba} iso={opt.iso}>
                <div style={{ display: "flex", alignItems: "center" }}>
                  <div>
                    <div style={{ color: "black" }}>{opt.dba}</div>
      ...
      <Row justify="end" align="middle">
        <Col span={6} style={{ display: "flex", justifyContent: "flex-end" }}>
          <Button
            ...
            disabled={
              !midData || midData.length === 0 || currentTableData.length === 0
            }
          >
      ...
      <Table
        loading={isUniqueMidsLoading || midDataLoading}
        columns={columns}
        onChange={(
